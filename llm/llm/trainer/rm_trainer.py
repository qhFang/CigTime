...
class RewardModelTrainer(ABC):
    ...
    def fit(self, args):
        ...
        for epoch in range(self.epochs):
            ...
            acc_mean = 0
            loss_mean = 0
            for full_ids, full_masks, rewards in self.train_dataloader:
                full_ids = full_ids.squeeze(1).to(torch.cuda.current_device())
                full_masks = full_masks.squeeze(1).to(torch.cuda.current_device())
                all_values, output = self.model(full_ids, attention_mask=full_masks, return_output=True)
                full_rewards = all_values[: full_ids.shape[0]]
                aux_loss = output.aux_loss if "aux_loss" in output else []

                # loss function
                if self.compute_fp32_loss:
                    full_rewards = full_rewards.float()

                preference_loss = self.mse_loss(full_rewards, rewards.to(torch.cuda.current_device()).type(full_rewards.dtype))
                # mixtral
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for full_ids, full_masks, rewards in self.train_dataloader:
                ...
                all_values, output = self.model(full_ids, attention_mask=full_masks, return_output=True)
                full_rewards = all_values[: full_ids.shape[0]]
                loss = self.mse_loss(full_rewards, rewards.to(torch.cuda.current_device()).type(full_rewards.dtype))
