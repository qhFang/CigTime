...
class Text2MotionDatasetEditGT(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer, gt_file, text_file):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
        ...
        ori_datas = joblib.load(open(gt_file, 'rb'))
        llm_texts = joblib.load(open(text_file, 'rb'))

        length_list = []
        new_name_list = []
        for i in range(len(llm_texts)):
            text = llm_texts[i]['tokens'].split('#')[0].strip()
            token = llm_texts[i]['tokens'].split('#')[1].strip()
            
            length = ori_datas['lengths'][i]
            motion = ori_datas['output_motion'][i][0][:length].cpu().numpy()
            if np.isnan(motion).any():
                continue
            motion = ori_datas['motion'][i][0][:length]
    ...
    def __getitem__(self, item):
        ...
        for token in tokens:
            try:
                ...
                print('Errors with the text tokens:', token)
                exit()
            pos_one_hots.append(pos_oh[None, :])
            word_embeddings.append(word_emb[None, :])
        pos_one_hots = np.concatenate(pos_one_hots, axis=0)
...
class Text2MotionDatasetEvalEdit(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer, results_file, text_file):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
        ...
        ori_datas = joblib.load(open(results_file, 'rb'))
        llm_texts = joblib.load(open(text_file, 'rb'))
        length_list = []
        new_name_list = []
        for i in range(len(llm_texts)):
            ...
            length = ori_datas['lengths'][i]
            motion = ori_datas['input_motion'][i][0][:length].cpu().numpy()
            if np.isnan(motion).any():
                continue
            motion = ori_datas['motion'][i][0][:length]
...
class Text2MotionDatasetEdit(data.Dataset):
    def __init__(self, opt, mean, std, split_file, edit_file, w_vectorizer):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
    ...
    def __getitem__(self, idx):
        data = self.data_dict[idx]
        motion, caption = data['motion'], self.edit_texts[idx % len(self.edit_texts)].strip()
        m_length = len(motion)
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", edit_file=None, gt_file=None, result_file=None, text_file=None, **kwargs):
        self.mode = mode
        ...
        elif mode in ['train', 'eval', 'text_only', 'edit']:
            # used by our models
            self.mean = np.load(pjoin(opt.data_root, f'Mean.npy'))
            self.std = np.load(pjoin(opt.data_root, f'Std.npy'))
        ...
        if mode == 'text_only':
            ...
            if mode == 'gt':
                self.t2m_dataset = Text2MotionDatasetEditGT(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer, self.gt_file)
            elif mode == 'edit':
                self.t2m_dataset = Text2MotionDatasetEdit(self.opt, self.mean, self.std, self.split_file, self.edit_file, self.w_vectorizer)
            elif mode == 'evalgt':
                self.t2m_dataset = Text2MotionDatasetEvalEdit(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer, self.result_file, self.text_file)
