...
class NaiveExperienceMaker(ABC):
    ...
    def detokenize_fn(self, tokens):
        ...
        for token_stream in tokens:
            ...
            batch.append(output_str)
        return batch



    @torch.no_grad()
    def make_experience(self, prompts: Union[str, List[str]], input_motions: torch.Tensor, output_motions: torch.Tensor, lengths: torch.Tensor, **generate_kwargs) -> Experience:
        self.actor.eval()
        self.critic.eval()
        ...
        ### Data augmentation ###
        input_motions2, output_motions2, prompts2, mask2, lengths2 = self.reward_model.random_sample(input_motions.permute(0, 2, 1).unsqueeze(2).cuda().bfloat16())
        input_motions = torch.cat([input_motions.cuda(), input_motions2.cuda()], dim=0)
        output_motions = torch.cat([output_motions.cuda(), output_motions2.cuda()], dim=0)
        prompts = prompts + prompts2
        lengths = torch.cat([lengths.cuda(), lengths2.cuda()], dim=0)
        ...
        descriptions = self.tokenizer.batch_decode(sequences, skip_special_tokens=True)
        #print(descriptions)
        for i in range(len(descriptions)):
            descriptions[i] = descriptions[i][len(prompts[i]):]
        # rewards
        input_motions = input_motions.permute(0, 2, 1).unsqueeze(2).cuda().bfloat16()
        output_motions = output_motions.permute(0, 2, 1).unsqueeze(2).cuda().bfloat16()

        genereted_output_motions, mask = self.reward_model(descriptions, input_motions, lengths)
        r = -F.mse_loss(genereted_output_motions, output_motions, reduction='none')
        r = r.squeeze(2).permute(0,2,1).mean(dim=-1)
        r = r * mask.float()
        r = r.reshape(genereted_output_motions.shape[0], -1).sum(dim=1) / mask.float().sum(dim=1)
        r[torch.isnan(r)] = -100
        ...
        return Experience(
            ...
            action_mask,
            info,
        ), input_motions, genereted_output_motions, output_motions, descriptions
